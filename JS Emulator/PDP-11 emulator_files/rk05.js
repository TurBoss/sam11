...
function rkrwsec(t) {
  ...
  for (i = 0; i < 256 && RKWC; i++) {
    ...
    } else memory[RKBA >> 1] = rkdisk[pos] | (rkdisk[pos + 1] << 8);
    RKBA += 2;
    pos += 2;
    RKWC = (RKWC + 1) & 0xffff;
  }
  ...
  if (sector > 013) {
    ...
    if (surface > 1) {
      surface = 0;
      cylinder++;
      if (cylinder > 0312) rkerror(RKOVR);
    }
  }
...
function rkgo() {
  switch ((RKCS & 017) >> 1) {
    ...
    case 2:
      ...
      break;
    default:
      panic("unimplemented RK05 operation " + ((RKCS & 017) >> 1).toString());
  }
}

function rkwrite16(a, v) {
  switch (a) {
    case 0777400:
      break;
    case 0777402:
