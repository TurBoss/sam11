...
function vt11Refresh()
{  // all drawing happens in background canvas which is periodically moved to the foreground
    if (vt11.refreshOn)
    {
...
function vt11Execute()
{
    ...
    for (count = 0; count < 8000; count++)
    {
        if ((instruction = vt11Read(vt11.DPC)) >= 0)
        {
            ...
            {                                   // data vs control
                ...
                if (vt11.DEBUG)
                    console.log((vt11.DPC - 2).toString(8) + " " + instruction.toString(8) + " Data mode " + mode.toString(2) + " I " + (intensify >> 14) + " X " + vt11.XRegister + " Y " + vt11.YRegister);
                switch (mode)
                {
                ...
                case 3:  // point mode
                    ...
                    if ((instruction = vt11Read(vt11.DPC)) >= 0)
                    {
                        ...
                        if (vt11.DEBUG)
                            console.log((vt11.DPC - 2).toString(8) + " " + instruction.toString(8) + " Extend " +
                ...
                {
                    penHit = 0;
                    if (!(vt11.DSR & 0x8) || vt11.blinkFlag)
                    {  // if not blinking or in blink cycle paint this...
                        ...
                        {  // character mode
                            ...
                            if (vt11.DEBUG)
                                console.log((vt11.DPC - 2).toString(8) + " write text " + String.fromCharCode(instruction & 0x7f, (instruction >> 8) & 0x7f));
                        }
                        else
                        {
                            if (intensify)
            ...
            else
            {
                ...
                if (vt11.DEBUG)
                    console.log((vt11.DPC - 2).toString(8) + " " + instruction.toString(8) + " Control mode " + mode.toString(2));
                if (mode <= 7)
                {  // set graphics mode
                    if (mode != 7)
                    {  // mode
                ...
                else
                {
                    ...
                    case 0xc:  // jump
                        ...
                        {
                            if (vt11.DEBUG)
                                console.log((vt11.DPC).toString(8) + " " + instruction.toString(8) + " Jump to " + instruction.toString(8));
                            vt11.DPC = instruction;
                        }
                        break;
                    case 0xe:  // status register a
                        if (instruction & 0x200)
                        {  // set stop interrupt enable
                            vt11.stopInterrupt = instruction & 0x100;
                        }
                        if (instruction & 0x80)
                        {                                                                  // set LP intensity hit register?
                            vt11.DSR = (vt11.DSR & 0xff7f) | ((instruction << 1) & 0x80);  // set LP flag (0=on, 1=off!!!)
                        }
                        if (instruction & 0x20)
                        {                      // set italics
                            instruction&0x10;  // Oops - something missed here?
                        }
                    ...
                    case 0xf:  // status register b
                        if (instruction & 0x40)
                        {  // set graphplot increment register
    ...
    if (vt11.DEBUG)
        console.log((vt11.DPC).toString(8) + " **end** X " + vt11.XRegister + " Y " + vt11.YRegister);
...
function accessVT11(physicalAddress, data, byteFlag)
{
    ...
    case 0o17772000:  // vt11.DPC
        ...
        if (result >= 0)
        {
            ...
            {
                if (vt11.DEBUG)
                    console.log((vt11.DPC).toString(8) + " **write to DPC** " + result.toString(8) + " X " + vt11.XRegister + " Y " + vt11.YRegister);
                if (!(result & 1))
                {
                    vt11.DPC = result;
                }
                if ((vt11.DSR & 0x8000))
                {  // need better track of whether active
                    vt11Execute();
                }
            }
        }
        break;
    case 0o17772002:  // vt11.DSR
        result = vt11.DSR;
        break;
    case 0o17772004:                                                          // light pen Y  ?? fix to add top bits
        result = (vt11.penX & 0x3ff) | ((vt11.graphIncrement & 0x2f) << 10);  // return light pen X location
        break;
    case 0o17772006:                 // light pen X  ?? fix to add top bits
