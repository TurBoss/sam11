...
const FPPsignMask = 0x8000,  // sign bit mask
...
function FPP_INSTRUCTION(a, b, c)
...
function fppFlags()
{  // 8 - N,  4 - Z,  2 - V,  1 - C Copy FPP flags to CPU flags
    "use strict";
    setFlags(0xf, FPP.FPS);  // Set CPU flags based on FPS flags
}

function fppZero(number)
{  // Zero a FPP number
    "use strict";
    var i;
...
function fppCopy(number, operand)
{  // Copy a FPP number
    "use strict";
    var i;
    for (i = FPP.precisionLength - 1; i >= 0; i--)
...
function fppTest(number)
{  // Test a FPP number (only Z or N can be set)
    "use strict";
...
function fppTestInt(operand)
{  // Test an integer operand (only Z or N can be set)
    "use strict";
    FPP.FPS &= 0xfff0;  //   8 - N,  4 - Z,  2 - V,  1 - C
    if (operand < 0)
        FPP.FPS |= 8;  // N Bit
...
function fppTrap(code)
{  // Trap code called by individual trap functions
    "use strict";
    FPP.FPS |= 0x8000;  // Set FER - floating point error
    FPP.FEC = code;
    FPP.FEA = (FPP.backupPC - 2) & 0xffff;
    if (!(FPP.FPS & 0x4000))
    {                       // Check FID - floating interrupt disable
        CPU.trapMask |= 8;  // Tell main emulator to FPP trap
    }
    return -1;
}

function fppPack(number, numberExp, numberSign)
{  // Routine to pack sign and pre-biased exponent back into a number
    "use strict";
...
function fppLDEXP(number, numberExp)
{  // Routine to load a new exponent into a FPP number
    "use strict";
    var numberSign;
...
function fppCompare(src1, src2)
{  // Compare floating point numbers: 0 if =, -ve if src1 < src2, +ve if src1 > src2
    "use strict";
    var i, result = 0;
    FPP.FPS &= 0xfff0;
    if ((src1[0] | src2[0]) & FPPexpMask)
    {  // If both exponents zero then finished!
        if ((src1[0] ^ src2[0]) & FPPsignMask)
        {  // For different signs + is larger
            result = 1;
        }
        else
...
function fppCompareWords(number, operand)
{  // Compare fraction only words giving: -ve number less, 0 equal, +ve number greater
    "use strict";
    var i, result = 0;
...
function fppAddSmall(number, small)
{  // Add a small constant to fraction only words
    "use strict";
    var i, carry = small;
...
function fppIncrement(number)
{  // Increment a full FPP number (unpack, add 1 to fraction, repack)
    "use strict";
...
function fppShiftLeft(number, shift)
{  // Shift fraction only words left in place
    "use strict";
...
function fppShiftRight(number, shift)
{  // Shift fraction only words right in place returning dropped bit
    "use strict";
    var i, wordIndex, bitShift, bitField = 0,
                                guardBit = 0;
    if (shift > 0)
...
function fppCopyShiftLeft(number, source, shift)
{  // Copy shifting fraction only words left
    "use strict";
    var i, wordIndex, bitShift, bitField = 0;
    wordIndex = ~~(shift / FPPwordBits);
    bitShift = FPPwordBits - (shift % FPPwordBits);
...
function fppCopyRightShift(number, source, shift)
{  // Copy shifting fraction only words right (no overlap)
    "use strict";
...
function fppCopyRightBits(number, source, shift, end)
{  // Copy fraction only words right (no overlap) with limit
    "use strict";
...
function fppFindNormal(number, start)
{  // Find where the normal point is (first bit set from a starting point)
    "use strict";
    var i, bit, word;
...
function fppSubtractWords(number, operand, guardBit)
{  // Subtract one set of fraction only words from another
    "use strict";
...
function fppAddWords(number, operand, guardBit)
{  // Add two sets of fraction only words
    "use strict";
    var i, carry = guardBit;
    for (i = FPP.precisionLength - 1; i >= 0; i--)
    {
...
function fppAdd(number, operand)
{  // Add two FPP numbers (number = number + operand)
    "use strict";
    ...
    if (!operandExp)
    {  // If operand is zero return number value
        if (!numberExp)
        {  // If number is also zero ensure it is exact zero
            fppZero(number);
        }
        fppTest(number);
    }
    ...
    {
        if (!numberExp)
        {  // If number is zero return operand value
            fppCopy(number, operand);
            fppTest(number);
        }
        else
        {  // Extract sign and exponent from operands, add the fractions, renormalize, pack...
            guardBit = 0;
            numberSign = number[0] & FPPsignMask;
            operandSign = operand[0] & FPPsignMask;
            number[0] = (number[0] & FPPfractionMask) | FPPhiddenMask;
            operand[0] = (operand[0] & FPPfractionMask) | FPPhiddenMask;
            if (!(numberSign ^ operandSign))
            {  // Same sign so ADD the two fractions
                if (numberExp != operandExp)
                {  // Different magnitude - need shift to align
                    if (numberExp < operandExp)
                    {
                ...
                if (number[0] & 0xff00)
                {  // Need to normalize maximum of one bit...
                    if (!(FPP.FPS & 0x20) && (number[FPP.precisionLength - 1] & 0x1))
                    {  // Add one if rounding and dropping a bit
                        fppAddSmall(number, 1);
                    }
                    fppShiftRight(number, 1);
                    numberExp++;
            ...
            {  // Opposite sign so SUBTRACT the two fractions
                if (numberExp == operandExp)
                {  // Same magnitude so determine larger fraction
                    largerNumber = fppCompareWords(number, operand);
                    if (!largerNumber)
                    {  // If fractions are identical return exact zero
                        fppZero(number);
                        fppTest(number);
                        return;
                    }
                ...
                {  // Different magntitude - need shift to align
                    if (numberExp < operandExp)
                    {
                        guardBit = fppShiftRight(number, operandExp - numberExp);
                        numberExp = operandExp;
                ...
                if (largerNumber < 0)
                {  // If number is smaller subtract it from operand
                    fppSubtractWords(operand, number, guardBit);
                    fppCopy(number, operand);
                    numberSign = operandSign;
                }
                else
                {  // If operand is smaller subtract it from number
...
function fppMultiply(number, operand)
{  // Multiply two FPP numbers (number = number x operand)
    "use strict";
    var i, j, carry, numberExp, operandExp, numberSign;
    numberExp = (number[0] & FPPexpMask) >>> FPPexpShift;
    operandExp = (operand[0] & FPPexpMask) >>> FPPexpShift;
    if (!operandExp || !numberExp)
    {  // If either number is zero return exact zero
        fppZero(number);
        fppTest(number);
    }
    else
    {  // Extract sign and exponent from operands, multiply the fractions, renormalize, pack...
        ...
        operand[0] = (operand[0] & FPPfractionMask) | FPPhiddenMask;
        for (i = FPP.precisionLength - 1; i >= 0; i--)
        {  // Multiply number x operand giving result
            carry = 0;
        ...
        if (!(FPP.FPS & 0x20))
        {  // If rounding look at bit to be dropped and round if necessary...
...
function fppDivide(number, operand)
{  // Divide two FPP numbers (number = number / operand)
    "use strict";
    var o, i, carry, divisor, qhat, numberExp, operandExp, numberSign;
    operandExp = (operand[0] & FPPexpMask) >>> FPPexpShift;
    if (!operandExp)
    {                // If divide by zero then trap
    ...
    else
    {
        numberExp = (number[0] & FPPexpMask) >>> FPPexpShift;
        if (!numberExp)
        {  // If number is zero return exact zero
            fppZero(number);
            fppTest(number);
        }
        else
        {  // Extract sign and exponent from operands, divide the fractions, renormalize, pack...
            numberSign = (number[0] ^ operand[0]) & FPPsignMask;
            ...
            if (fppCompareWords(number, operand) < 0)
            {  // Shift number left and ensure it is larger than operand
            ...
            if (!(FPP.FPS & 0x20))
            {  // If rounding compute next digit...
                qhat = ~~(((FPP.result[o - 1] * FPPwordBase + FPP.result[o]) * FPPwordBase + FPP.result[o + 1]) / divisor);
                if (qhat >= FPPwordBase)
                    qhat = FPPwordBase - 1;
                if (qhat & 0x8000)
...
function fppMODF(number, whole, operand)
{  // number = fraction part of number x operand - whole the optional integer part
    "use strict";
    ...
    {  // Extract sign and exponent from operands, multiply the fractions, extract integer and fraction components, pack...
        for (i = FPP.result.length - 1; i >= 0; i--)
        {
            FPP.result[i] = 0;  // Initialize result
        }
        ...
        {                                       // FT - Truncate (not round)
            ...
            if (position < 8)
            {
                ...
                if (FPP.result[FPP.precisionLength - 1] & (0x80 >>> position))
                {                                                 // Check appropriate extra bit
        ...
        if (whole != null)
        {  // If the optional integer component is required it comes from the start of the result
        ...
        if (numberExp - position >= 0x100)
        {  // Check for exponent overflow which results in returning the fraction as zero
            fppZero(number);
        ...
        {
            if (numberExp - position > FPPexpBias)
            {  // If there is an integer component find the start of the fraction
                position = fppFindNormal(FPP.result, numberExp - FPPexpBias);
            }
            if (position < 0 || position >= 10 + (FPP.precisionLength - 1) * FPPwordBits)
            {  // If no fraction or beyond limit of FP11 return zero
                fppZero(number);
                fppTest(number);
                return;
            }
            else
            {  // Normalize fraction right or left as appropriate
                if (position != 8)
                {
                    if (position < 8)
                    {
...
function fppLDCIF(number, addressMode)
{  // Load converting from integer to FPP
    "use strict";
    var result, src, virtualAddress, numberSign, numberExp = FPPwordBits,
                                                 length = 2;
    result = 0;
    if (!(addressMode & 0x38))
    {  // If register mode use 16 bit signed resgister value
        src = CPU.registerVal[addressMode & 7] << FPPwordBits;
    }
    else
    {  // Get a signed 16 or 32 bit value from memory
        if ((FPP.FPS & 0x40) && (addressMode & 0x3f) != 0x17)
        {  // (addressMode & 077) != 027) { // FL - Long integer mode
            length = 4;
        }
    ...
    if (result >= 0)
    {
        ...
        {  // Non zero requires sign extraction and normalization
            numberSign = 0;
            if (src & 0x80000000)
            {
            ...
            if (!(src & 0xff000000))
            {  // Normalize smaller integer by shifting left to make a two fraction FPP number
                numberExp -= 8;
            ...
            {  // Normalze a larger integer by creating a 32 bit value from which we extract 3 fraction components
...
function fppSTCFI(number, addressMode)
{  // Store converting from floating to integer
    "use strict";
    var result = 0,
        words = 1,
    ...
    {  // Build fraction bits into 32 bit integer and shift to get integer result
        ...
        else
        {
            ...
            {  // Overflow condition - result is zero
                result = 0;
                if (FPP.FPS & 0x100)
                {                // FIC - Floating point conversion trap
    ...
    if (!(addressMode & 0x38))
    {  // Register mode (throw away bottom 16 bits)
...
function writeFloatByVirtual(virtualAddress, number)
{  // Write FPP number by virtual address (17 bit I&D)
    "use strict";
    var result, i;
    for (i = 0; i < FPP.modeLength; i++)
    {  // FPP.modeLength set by prior call to getFloatVirtualByMode()
        if ((result = writeWordByVirtual(virtualAddress, number[i])) < 0)
        {  // Write each word of the FPP number
            break;
        }
        virtualAddress = incrementVirtual(virtualAddress);
    }
...
function readFloatByVirtual(number, virtualAddress)
{  // Read FPP number by virtual address (17 bit I&D)
    "use strict";
    var result, i;
    for (i = 0; i < FPP.modeLength; i++)
    {  // FPP.modeLength set by prior call to getFloatVirtualByMode()
        if ((result = readWordByVirtual(virtualAddress)) < 0)
        {  // Read in each word of the FPP number
            break;
        }
    ...
    if (result >= 0)
    {  // If all ok zero fill any remaining words
    ...
    if (result >= 0 && (number[0] & FPPsignMask) && !(number[0] & FPPexpMask))
    {  // Is it -0?
        if (FPP.FPS & 0x0800)
        {                 // Are undefined variable traps enabled?
...
function getFloatVirtualByMode(addressMode)
{  // Determine Virtual Address for instruction mode
    "use strict";
    if ((addressMode & 0x3f) == 0x17)
    {                        // (addressMode & 077) == 027 or (PC)+ as in MOV #xx,....
...
function writeFloatByMode(addressMode, number)
{  // Write FPP number by instruction mode
    "use strict";
    var result;
    if (!(addressMode & 0x38))
    {                      // If register mode write to register
    ...
    {
        if ((result = getFloatVirtualByMode(addressMode)) >= 0)
        {                                                  // (mode sets FPP.modeLength)
...
function readFloatByMode(number, addressMode)
{  // Read FPP number by instruction mode
    "use strict";
    var virtualAddress, result;
    if (!(addressMode & 0x38))
    {                      // If register mode copy from register
        addressMode &= 7;  // Now just register number
        if (addressMode < 6)
        {
    ...
    {
        if ((virtualAddress = getFloatVirtualByMode(addressMode)) < 0)
        {  // (mode sets FPP.modeLength)
...
function modifyFloat(number)
{  // Update last FPP number read
    "use strict";
...
function executeFPP(instruction)
{  // Main entry point call by mainline emulation when a FPP instruction is encountered
    "use strict";
    var AC,
      result,
      virtualAddress;
    ...
    case 0x100:  // 0000400 Single operand FPP instructions
        ...
        case 2:  // 02 ABSF Make Absolute Floating/Double
            ...
            if (readFloatByMode(FPP.scratch, instruction) != -1)
            {  // Allow for undefined variable trap (-2)
    ...
    case 0x300:  // 0001400 MODF Multiply and integerize Floating/Double
        ...
        {
            if (AC & 1)
            {  // Special case for odd AC where integer result is not returned (we have nowhere to put it)
    ...
    case 0xc00:  // 0006000 STCFD Store converting Floating/Double to Double Floating
        ...
        if (FPP.precisionLength == 4)
        {  // Writing double so it needs to be zero extended from real
            FPP.scratch[2] = FPP.scratch[3] = 0;
        }
        else
        {  // Double to Real may require rounding
            if (!(FPP.FPS & 0x20) && (FPP.scratch[2] & 0x8000))
            {                               // not FT - Truncate and dropping significant bit
    ...
    case 0xf00:  // 0007400 LDCDF Load converting Floating/Double to Double Floating
        ...
        if (readFloatByMode(FPP.AC[AC], instruction) >= 0)
        {
            FPP.precisionLength = 6 - FPP.precisionLength;  // Restore precision
            if (FPP.precisionLength == 4)
            {  // In double mode zero fill from real
                FPP.AC[AC][2] = FPP.AC[AC][3] = 0;
                fppTest(FPP.AC[AC]);
            }
            else
            {  // In real mode truncate from double with rounding if needed
                if (!(FPP.FPS & 0x20) && (FPP.AC[AC][2] & 0x8000))
                {                              // FT - Truncate
