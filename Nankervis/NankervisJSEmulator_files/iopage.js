...
//
//
const IO_BLOCKSIZE = 1024 * 1024;  // 1 Mb request size. Larger reduces number of requests but increases count
...
function diskIO(operation, meta, position, address, count)
{
    ...
    if (typeof meta.cache[block] !== "undefined")
    {
        ...
        while (count > 0)
        {
            ...
            case 2:  // Read: read to memory from cache
                ...
                if (count > 1)
                {  // tape can read odd number of bytes - of course it can. :-(
    ...
    if (count > 0)
    {  // I/O not complete so we need to get some data
...
function accessRK11(physicalAddress, data, byteFlag)
{
    ...
    case 0o17777404:  // rk11.rkcs
        ...
        if (data >= 0 && result >= 0)
        {  // writing rkcs?
            if ((rk11.rkcs ^ result) & 0x40)
            {  // Has IE bit changed?
                if ((result & 0x40))
                {  // If IE bit now set then interrupt
                    //                      interrupt(0, 5 << 5, 0o220, 0); //  Interrupt!
                }
            }
            rk11.rkcs = (result & ~0xf080) | (rk11.rkcs & 0xf080);  // Bits 7 and 12 - 15 are read only
            if ((rk11.rkcs & 0x81) === 0x81)
            {                        // If done & go are both set then kick off new work...
...
function rl11_finish(drive)
{
    ...
    {  // if interrupt not enabled just mark completed
        rl11_commandEnd();
    }
}
...
function accessRL11(physicalAddress, data, byteFlag)
{
    ...
    case 0o17774400:  // rl11.csr
        ...
        if (data >= 0 && result >= 0)
        {
            if ((rl11.csr & 0x40) && !(result & 0x40))
            {  // if IE being reset then kill any pending interrupts
                //rl11.csr |= 0x81; // turn off go & set ready
...
function tm11_go()
{
    ...
    switch (tm11.meta[drive].command)
    {        // function code
    case 0:  // off-line
        break;
    case 1:  // read
...
function accessPTR11(physicalAddress, data, byteFlag)
{
    ...
    case 0:  //  ptr11.prs  017777550
        ...
        if (result >= 0 && data > 0)
        {
            ...
            if (!(ptr11.prs & 0x8800) && (result & 1))
            {  // If not ERROR or BUSY and setting GO...
                if (typeof ptr11.meta === "undefined")
                {  // Make metadata if not there
                    ptr11.meta = {
                        "cache" : [],
...
{
    document.getElementById("lp11").innerHTML = '<p>printer<br /><textarea id=lp11_id cols=132 rows=24 spellcheck=false style="font-family:Liberation Mono,Monaco,Courier New,Lucida Console,Consolas,DejaVu Sans Mono,Bitstream Vera Sans Mono,monospace"></textarea><br /><button onclick="document.getElementById(' +
                                                "'lp11_id'" +
                                                ').value=' +
                                                "''" +
                                                ';">Clear</button></p>';
...
{  // Called when a new terminal identified
    "use strict";
    var divElement;
    if (unit !== 0)
    {
        divElement = document.createElement('div');
        divElement.innerHTML = '<p>tty' + unit + '<br /><textarea id=' + unit + ' cols=132 rows=24 style="font-family:' +
                               "'Courier New'" +
                               ',Courier,' +
                               "'Lucida Console'" +
                               ',Monaco,monospace;" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false"></textarea><br /></p>';
...
function dl11Input(unit, ch)
{
    ...
    {              // Done set - last character not yet used
...
function accessDL11(physicalAddress, data, byteFlag, unit, vector)
{
    ...
    case 6:  // DL xbuf
        ...
        if (data >= 0 && result >= 0)
        {
            ...
            if (DL11[unit].xcsr & 0x40)
            {  // Cheat: leave Done permanently set
    ...
    case 0:  // DL rcsr
        ...
        if (result >= 0 && data >= 0)
        {
            if ((DL11[unit].rcsr & 0x40) && !(result & 0x40))
            {  // Did IE just get turned off?
...
function kw11_init()
{
    "use strict";
    kw11.csr = 0x80;
    if (kw11.timerId === null)
    {  // If not initialized set timer for every 20ms (50Hz)
        kw11.timerId = setTimeout(kw11_interrupt, 20);
    }
}

function kw11_interrupt()
{  // Called every 20 ms (50 Hz) to check whether time exhausted
    "use strict";
    var timeNow = Date.now();
    kw11.interruptTime += 20;
    if (timeNow - kw11.interruptTime > 30000)
    {  // Try to time accurately but give up if 30 seconds behind
...
{  // access_iopage() handles all I/O page requests
    "use strict";
    var result, idx;
    switch (physicalAddress & ~0o77)
    {                 // Break addressing up into blocks with common lower 6 bits
    case 0o17777700:  // 017777700 - 017777777 First block is highest addresses including PSW, stack limit, PIR, etc
        ...
        case 0o17777772:  // PIR
            ...
            if (result >= 0 && data >= 0)
            {
                result &= 0xfe00;
                if (result)
                {  // Need to calculate priority level from priority mask
                    idx = result >>> 9;
                    do
                    {
                        result += 0x22;
        ...
        case 0o17777740:       // Low error address
            ...
            else
            {
                ...
                if (result >= 0)
                {
                    if ((physicalAddress & ~1) === 0o17777746)
                        result = 0o17;
                    if ((physicalAddress & ~1) === 0o17777742)
                        result = 0o3;
                    if ((physicalAddress & ~1) === 0o17777740)
                        result = 0o177740;
        ...
        case 0o17777710:  // Register set 1
            ...
            {
                if (data >= 0)
                    CPU.registerVal[idx] = data;
                result = CPU.registerVal[idx];
            }
            else
            {
                if (data >= 0)
                    CPU.registerAlt[idx] = data;
    ...
    case 0o17777500:  // 017777500 - 017777577 MMR0 MMR1 MMR2 Console KW11
        ...
        case 0o17777546:  // kw11.csr
            ...
            {
                result &= 0x40;
                if ((result ^ kw11.csr) & 0x40)
                {  // Is IE changing?
                    if (result & 0x40)
                    {                    // If turning on interrupt now for diags (otherwise timing is too slow)
    ...
    case 0o17770200:  // 017770200 - 017770277 Unibus Map
        ...
        else
        {
            ...
            if (physicalAddress & 0o2)
                result = (result >>> 16) & 0x803f;  // Low six bits plus top bit (!)
    ...
    if (byteFlag && result >= 0)
    {  // Make any required byte adjustment to the return result
        if ((physicalAddress & 1))
        {
            result = result >>> 8;
        }
    ...
    if (result < 0)
    {                              // on failure set Address Error light
