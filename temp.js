...
function mapUnibus(unibusAddress) {
  ...
  if (idx < 31) {
    if (CPU.MMR3 & 0x20) {
      unibusAddress =
        (CPU.unibusMap[idx] + (unibusAddress & 0x1fff)) & 0x3fffff;
    }
  } else {
      ...
      if (typeof meta.cache[block] !== "undefined") {
        ...
        for (word = 0; word < meta.blockSize; word++) {
          if (base < byteArray.length) {
            meta.cache[block][word] =
              (byteArray[base] & 0xff) | ((byteArray[base + 1] << 8) & 0xff00);
          } else {
...
function diskIO(operation, meta, position, address, count) {
      ...
      switch (operation) {
        ...
        case 2: // Read: read to memory from cache
          if (count > 1) {
            ...
            if (
              writeWordByAddr(
                meta.mapped ? mapUnibus(address) : address,
                meta.cache[block][word]
              ) < 0
            ) {
              meta.postProcess(
  ...
  if (count > 0) {
    ...
    data =
      ~~((position + (count >> 1) + meta.blockSize - 1) / meta.blockSize) *
      meta.blockSize; // End word
    xhr = new XMLHttpRequest();
    xhr.open("GET", meta.url, true);
    xhr.setRequestHeader(
      "Range",
      "bytes=" + (word << 1) + "-" + ((data << 1) - 1)
    );
...
function rk11_go() {
  ...
  if (rk11.TRACKS[drive] == 0) {
    ...
    sector =
      ((rk11.rkda >> 4) & 0x1ff) * rk11.SECTORS[drive] + (rk11.rkda & 0xf);
...
function rk11_end(err, meta, position, address, count) {
  ...
  rk11.rkda =
    (rk11.rkda & 0xe000) |
    (~~(position / rk11.SECTORS[meta.drive]) << 4) |
    position % rk11.SECTORS[meta.drive];
  switch (err) {
    case 1: // read error
  ...
  switch (
    ...
    case 3: // seek
      if ((rl11.dar & 3) == 1) {
        if (rl11.dar & 4) {
          rl11.DAR =
            ((rl11.DAR + (rl11.dar & 0xff80)) & 0xff80) |
            ((rl11.dar << 2) & 0x40);
        } else {
          rl11.DAR =
            ((rl11.DAR - (rl11.dar & 0xff80)) & 0xff80) |
...
function rl11_end(err, meta, position, address, count) {
  ...
  rl11.dar =
    (~~(sector / rl11.SECTORS[meta.drive]) << 6) |
    sector % rl11.SECTORS[meta.drive];
  rl11.DAR = rl11.dar;
...
function rp11_go() {
  ...
  if (typeof rp11.meta[drive] === "undefined") {
    rp11.meta[drive] = {
      ...
      mapped: 0,
      maxblock:
        rp11.CYLINDERS[drive] * rp11.SURFACES[drive] * rp11.SECTORS[drive],
      url: "rp" + drive + ".dsk",
    };
  }
  switch (
    rp11.rpcs1 & 0x3f // function code
  ) {
    case 01: // NULL
      return;
    case 03: // unload
    ...
    case 061: // write
      if (
        rp11.rpdc[drive] >= rp11.CYLINDERS[drive] ||
        rp11.rpda[drive] >> 8 >= rp11.SURFACES[drive] ||
      ...
      sector =
        (rp11.rpdc[drive] * rp11.SURFACES[drive] + (rp11.rpda[drive] >> 8)) *
          rp11.SECTORS[drive] +
        (rp11.rpda[drive] & 0xff);
      diskIO(
        1,
        rp11.meta[drive],
        sector * 256,
        rp11.rpba,
        ((0x10000 - rp11.rpwc) & 0xffff) << 1
      );
      return;
      break;
    case 071: // read
...
function rp11_end(err, meta, position, address, count) {
  var sector,
    block = ~~((position + 255) / 256);
  rp11.rpwc = (0x10000 - (count >> 1)) & 0xffff;
  rp11.rpba = address & 0x3fffff;
  sector = ~~(block / rp11.SECTORS[meta.drive]);
  rp11.rpda[meta.drive] =
    (sector % rp11.SURFACES[meta.drive] << 8) |
    block % rp11.SECTORS[meta.drive];
...
function accessRP11(physicalAddress, data, byteFlag) {
  ...
  idx = rp11.rpcs2 & 7;
  switch (
    physicalAddress & ~1 // RH11 always there addresses
  ) {
    ...
    default:
      ...
      if (rp11.rpds[idx] & 0x100) {
        switch (
          physicalAddress & ~1 // Drive registers which may or may not be present
        ) {
...
function tm11_go() {
  ...
  switch (
    tm11.meta[drive].command // function code
  ) {
    case 0: // off-line
      break;
    case 1: // read
...
function kw11_interrupt() {
  // Every 20 ms (50 Hz) set clock flag and schedule an interrupt
  kw11.csr |= 0x80; // If halted don't interrupt as console commands would always be doing clock stuff
...
function access_iopage(physicalAddress, data, byteFlag) {
  ...
  switch (physicalAddress & ~077) {
      ...
      switch (physicalAddress & ~1) {
        case 017772516: // MMR3 - UB 22 x K S U
          ...
          if ((result >= 0) & (data >= 0)) {
            ...
            CPU.MMR3 = result;
            CPU.MMR3Mode =
              ((result & 4) >> 2) | (result & 2) | ((result & 1) << 2); // KSU bits in reverse order for ease of access
    ...
    case 017772200: // 017772200 - 017772277 MMU super mode 1 Map
      ...
      break;
    case 017772000: // 017772000 - 017772006 vt11 display
    ...
    case 017770200: // 017770200 - 017770277 Unibus Map
      ...
      if (result >= 0 && data >= 0) {
        if (physicalAddress & 02) {
          CPU.unibusMap[idx] =
            ((result & 0x803f) << 16) | (CPU.unibusMap[idx] & 0xfffe);
        } else {
          CPU.unibusMap[idx] =
            (CPU.unibusMap[idx] & 0x803f0000) | (result & 0xfffe);
        }
      }
      break;
    default:
